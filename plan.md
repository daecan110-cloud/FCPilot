...
- [x] 세로(Portrait) PDF 계약/보장금액 파싱 지원
- [x] ITEM_ROW_MAP 짧은 키 정리 (치매/수술비/사망 등 오매핑 차단)
- [x] 운전자보험 교통상해사망/특수수술비 무시 항목 추가
- [x] 2차 보완 가드 강화 (seen_rows 항목 0이어도 덮어쓰기 금지)
- [x] verify 함수 카테고리명 필터 + 페이지간 중복 방지
- [x] test_parsing.py v13 행번호 반영 → 5/5 PASS
- [x] test_pdf_batch.py 신규 (tests/pdfs/ 일괄 검증)
- [x] Git 프로젝트별 계정 분리

## Sprint 21 — 완료 (2026-04-26)
- [x] 전체 성능 최적화 — set_session/쿼리 캐싱/N+1 제거/fragment/folium 경량화 (10건)
- [x] 보안 수정 — API 키 제거/esc() 추가/Hook 패턴 강화 (3건)
- [x] 코드 품질 — 상수 통합/silent fail 경고/rerun 최적화 (3건)
- [x] DB 마이그레이션 — analysis_records client_id FK 추가
- [x] 문서 동기화 — SECURITY.md/RULES_DETAIL.md 정리
- [x] Memory 시스템 초기화 (MEMORY.md + 5개 메모리 파일)
- [x] 테스트 71/74 통과 (3건 기존 이슈)
...
| 20 | PDF 파싱 세로 지원 + 오매핑 수정 + 테스트 정비 | ✅ 완료 |
| 21 | 전체 성능 최적화 + 보안/품질 개선 (20건) | ✅ 완료 |
| 22 | 대형 리팩터링 (200줄 초과 파일 분리) | 2~3일 |
| — | 생일알림 + Daily Reminder + 자동백업 | 3~4일 |
| — | 보장분석 비교 + 셀 병합 검증 | 3~4일 |
| — | 상담 스크립트 + 약관 AI 대화창 + PWA | 1주 |
