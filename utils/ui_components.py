...
def empty_state(icon: str, message: str):
    ...
    st.markdown(
        ...
        f'<div style="font-size:36px; margin-bottom:8px;">{_html.escape(str(icon))}</div>'
        f'<div style="font-size:14px;">{_html.escape(str(message))}</div>'
        f'</div>',
...
def section_header(title: str, subtitle: str = ""):
    ...
    st.markdown(
        ...
        f'<span style="font-size:16px; font-weight:600; color:#1a1a2e;">{_html.escape(str(title))}</span>'
        + (
            f'<span style="font-size:13px; color:#9ca3af; margin-left:8px;">{_html.escape(str(subtitle))}</span>'
            if subtitle else ""
        )
        + '</div>',
        unsafe_allow_html=True,
